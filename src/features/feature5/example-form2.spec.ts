...
import ExampleForm2 from './example-form2.vue'

describe('example-form-2', () => {
  it('shows the form fields', () => {
    const { getByRole } = render(ExampleForm2)
  ...
  it('shows errors when submitting an empty form', async () => {
    const { getByRole, getByText } = render(ExampleForm2)
  ...
  it('shows no errors when inputting valid values', async () => {
    const { getByRole, queryByText } = render(ExampleForm2)
    const user = userEvent.setup()
